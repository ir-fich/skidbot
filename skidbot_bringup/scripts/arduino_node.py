...
#######################################################################################################################
		...
		self._left_front_encoder_value = 0
		self._left_rear_encoder_value = 0
		self._right_front_encoder_value = 0
		self._right_rear_encoder_value = 0
		...
		self._left_front_wheel_speed_ = 0
		self._left_rear_wheel_speed_ = 0
		self._right_front_wheel_speed_ = 0
		self._right_rear_wheel_speed_ = 0
...
#Subscribers and Publishers
		...
		#Publisher for left and right wheels encoder values
		self._Left_Front_Encoder = rospy.Publisher('lf_wheel',Int64,queue_size = 10)
		self._Left_Rear_Encoder = rospy.Publisher('lr_wheel',Int64,queue_size = 10)
		self._Right_Front_Encoder = rospy.Publisher('rf_wheel',Int64,queue_size = 10)
		self._Right_Rear_Encoder = rospy.Publisher('rr_wheel',Int64,queue_size = 10)
...
#Speed subscriber
		...
		self._left_front_motor_speed = rospy.Subscriber('left_front_wheel_speed',Float32,self._Update_Left_Front_Speed)
		self._left_rear_motor_speed = rospy.Subscriber('left_rear_wheel_speed',Float32,self._Update_Left_Rear_Speed)
		self._right_front_motor_speed = rospy.Subscriber('right_front_wheel_speed',Float32,self._Update_Right_Front_Speed)
		self._right_rear_motor_speed = rospy.Subscriber('right_rear_wheel_speed',Float32,self._Update_Right_Rear_Speed)


#######################################################################################################################
	def _Update_Left_Front_Speed(self, left_speed):

		# Falta separar las ruedas delanteras y traseras
		self._left_front_wheel_speed_ = left_speed.data

		rospy.loginfo(left_speed.data)

		speed_message = 's %d %d %d %d\r' %(int(self._left_front_wheel_speed_),int(self._left_rear_wheel_speed_),int(self._right_front_wheel_speed_),int(self._right_rear_wheel_speed_))

		self._WriteSerial(speed_message)

#######################################################################################################################
	def _Update_Left_Rear_Speed(self, left_speed):

		# Falta separar las ruedas delanteras y traseras
		self._left_rear_wheel_speed_ = left_speed.data
...
#######################################################################################################################################################3
	def _Update_Right_Front_Speed(self, right_speed):

		self._right_front_wheel_speed_ = right_speed.data
...
#######################################################################################################################################################3
	def _Update_Right_Rear_Speed(self, right_speed):

		self._right_rear_wheel_speed_ = right_speed.data
...
#Calculate orientation from accelerometer and gyrometer
	...
	def _HandleReceivedLine(self,  line):
		...
		if(len(line) > 0):
			...
			try:
				if(lineParts[0] == 'e'):
					self._left_front_encoder_value = long(lineParts[1])
					self._left_rear_encoder_value = long(lineParts[2])
					self._right_front_encoder_value = long(lineParts[3])
					self._right_rear_encoder_value = long(lineParts[4])

#######################################################################################################################

					self._Left_Front_Encoder.publish(self._left_front_encoder_value)
					self._Left_Rear_Encoder.publish(self._left_rear_encoder_value)
					self._Right_Front_Encoder.publish(self._right_front_encoder_value)
					self._Right_Rear_Encoder.publish(self._right_rear_encoder_value)
